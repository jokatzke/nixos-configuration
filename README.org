...
While almost the entirety of the source code blocks gets exported to the ~nix~ file =/etc/nixos/configuration.nix=, we can leverage the power of literate programming to write e.g. ~bash~ with all expected editing features in Emacs and then simply export them as block-comment to this file.

* Hardware Configuration

Since the hardware configuration in NixOS is generated automatically, it is not included here.
To generate it, run
#+begin_src shell
sudo nixos-generate-config
#+end_src
IMPORTANT: This overrides =/etc/nixos/configuration.nix=, which is the primary configuration file for NixOS!
Make sure to not have any edits in your current version of this file that are not present here as well.


* Flake
:PROPERTIES:
:header-args: :results none :tangle flake.nix
:END:

[[id:38364369-088a-47f1-b807-b5566d9a7b01][Nix Flakes]] appear to be the way forward for Nix(OS), and
#+begin_src nix
{
...
**** Installation
Install Emacs and run it as a service on startup.
- The gcc branch converts emacs-lisp into compiled C binaries, hugely improving performance.
- The pure gtk branch uses, as its name suggests, purely gtk instead of the default, and much older, lucid toolkit.
  This allows for hardware acceleration and better [[id:6b0d61dd-a425-494c-824c-2ce8cf202d09][Wayland]] integration, but can lead to instabilities, especially when using Emacs as a daemon,
  which we are also doing here.
#+name: emacs_install
#+begin_src nix
programs.emacs = {
  enable = true;
...
#+end_src
Running it as a services makes all future instances of Emacs start up immediately when used as clients,
rather than having to load up all packages every time (which can take multiple seconds).
...
The script below, which can be run automatically on file changes within the doom directory, syncs doom configuration changes.
#+begin_src nix
home.file.".doom.d" = {
  source = ./doom;
  recursive = true;
...
#+name: reload_doom
#+begin_src sh :tangle ./scripts/reload_doom.sh
#!/bin/sh
DOOM="$HOME/.emacs.d"
...
When a new iteration of the Qtile configuration has been linked by Home-Manager, the script to reload Qtile ([[reload_qtile]]) is called.
#+begin_src nix
...
#+name: reload_qtile
#+begin_src sh :tangle ./scripts/reload_qtile.sh
#!/bin/sh
